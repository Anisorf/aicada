...
class logostorage {
    ...
    public static function add_form_elements(&$mform) {
        $mform->addElement('static', '_logoman', '', get_string('logomanager:desc', 'block_couponext'));
    ...
    public static function add_select_form_elements(&$mform) {
        $mform->addElement('static', '_logo', '', get_string('select:logo:desc', 'block_couponext'));
        $mform->addElement('select', 'logo', get_string('select:logo', 'block_couponext'), static::get_file_menu());
    }
    ...
    public static function prepare_draftarea(&$draftitemid) {
        ...
        file_prepare_draft_area($draftitemid, $context->id, 'block_couponext',
    ...
    public static function store_draft_files($draftitemid) {
        ...
        $options = array('subdirs' => 0, 'maxfiles' => 10);
        $text = null;
        return file_save_draft_area_files($draftitemid, $context->id, 'block_couponext',
    ...
    public static function store_from_content($filename, $content) {
        ...
        $filerecord->component = 'block_couponext';
    ...
    public static function get_files() {
        $fs = get_file_storage();
        $context = \context_system::instance();
        if (!$files = $fs->get_area_files($context->id, 'block_couponext', self::FILE_AREA, self::AREA_ITEM_ID, 'id DESC', false)) {
            return false;
        }
        return $files;
    }
    ...
    public static function get_file_menu() {
        ...
        $context = \context_system::instance();
        $rs = array(
            -1 => get_string('logo:none', 'block_couponext'),
            0 => get_string('logo:default', 'block_couponext')
        );
        if (!$files = $fs->get_area_files($context->id, 'block_couponext', self::FILE_AREA, self::AREA_ITEM_ID, 'id DESC', false)) {
            return $rs;
        }
     ...
     *
     * @param int $fileid
     * @return \block_couponext\tempfile
     */
    static public function get_tempfile_for($fileid) {
        ...
        $parts = explode('.', $fn);
        $tempfile = \block_couponext\tempfile::create_from_content($file->get_content(), end($parts));
        return $tempfile;
    }
