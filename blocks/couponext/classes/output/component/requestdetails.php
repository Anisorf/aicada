...
namespace block_couponext\output\component;

defined('MOODLE_INTERNAL') || die();

/**
 * block_couponext\manager\requestdetails
 *
 * @package     block_couponext
 *
 * @copyright   Sebsoft.nl
...
class requestdetails implements \renderable, \templatable {
    ...
    public function export_for_template(\renderer_base $output) {
        ...
        if ($generatoroptions->logoid == -1) {
            $data->logo = get_string('logo:none', 'block_couponext');
        } else if ($generatoroptions->logoid == 0) {
            $data->logo = get_string('logo:default', 'block_couponext');
        ...
        if ($generatoroptions->enrolperiod == 0) {
            $data->enrolperiod = get_string('enrolperiod:indefinite', 'block_couponext');
        } else {
            $data->enrolperiod = format_time($generatoroptions->enrolperiod);
        }

        if ($generatoroptions->type == \block_couponext\coupon\generatoroptions::COURSE) {
            $data->type = get_string('label:type_course', 'block_couponext');
            ...
            $data->type = get_string('label:type_cohort', 'block_couponext');
