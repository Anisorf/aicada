...
/**
 * this file contains the table to display requestusers
 *
 * File         requestusers.php
 * Encoding     UTF-8
 *
...
/**
 * block_couponext\tables\requestusers
 *
 * @package     block_couponext
 *
 * @copyright   Sebsoft.nl
...
class requestusers extends \table_sql {
     ...
     * @return \block_couponext\tables\requestusers
     */
    ...
    public function render($pagesize, $useinitialsbar = true) {
        ...
        $from = '{block_couponext_rusers} cu ';
    ...
    public function col_action($row) {
        ...
        $aedit = \html_writer::link(new \moodle_url($this->baseurl,
                ['action' => 'edituser', 'itemid' => $row->id, 'sesskey' => sesskey()]),
                \html_writer::img($OUTPUT->image_url('i/edit'), 'Edit', ['class' => 'icon']));
        $actions[] = $aedit;
    ...
    protected function define_table_columns($columns) {
        ...
        foreach ($columns as $name) {
            $headers[] = get_string('th:' . $name, 'block_couponext');
        }
