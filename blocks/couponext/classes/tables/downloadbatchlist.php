...
/**
 * block_couponext\tables\downloadbatchlist
 *
 * @package     block_couponext
 *
 * @copyright   Sebsoft.nl
...
class downloadbatchlist extends \table_sql {
    ...
    public function __construct(\context $context, $ownerid = null) {
        ...
        $this->sortable(false);
        $this->strdownload = get_string('view:download:title', 'block_couponext');
    }
    ...
    public function render($pagesize, $useinitialsbar = true) {
        ...
        if (!has_capability('block/couponext:viewallreports', $this->context)) {
            $finalrows = [];
            $batchids = [];
            $tids = [];
            foreach ($rows as $row) {
    ...
    public function col_action($row) {
        ...
        $actions[] = $renderer->action_icon(
                new \moodle_url($CFG->wwwroot . '/blocks/couponext/download.php',
