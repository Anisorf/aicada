...
class errorreport extends \table_sql {
    ...
    public function __construct($ownerid = null) {
        ...
        $this->strdelete = get_string('action:error:delete', 'block_couponext');
    }
    ...
    protected function get_query($forcount = false) {
        ...
        $sql = 'SELECT ' . $fields . '
               ...
               JOIN {block_couponext_errors} e ON e.couponid=c.id';
    ...
    protected function define_table_columns($columns) {
        ...
        foreach ($columns as $name) {
            if ($name == 'batchid') {
                $headers[] = get_string('label:' . $name, 'block_couponext');
            } else {
