...
class report extends \table_sql {
    ...
    protected function get_query($forcount = false) {
        ...
        $q1 = 'SELECT ' . $fields . '
               , null as cohortname
               FROM {block_couponext} bc
               JOIN {block_couponext_courses} cc ON cc.couponid=bc.id
        ...
        $q2 = 'SELECT ' . $fields . '
               ...
               FROM {block_couponext} bc
               JOIN {block_couponext_cohorts} cc ON cc.couponid=bc.id
    ...
    public function col_type($row) {
        return get_string('coupon:type:' . $row->typ, 'block_couponext');
    }
    ...
    protected function define_table_columns($columns) {
        ...
        foreach ($columns as $name) {
            if ($name === 'fullname') {
                ...
                $headers[] = get_string('report:heading:couponcode', 'block_couponext');
            } else if ($name === 'typ') {
                $headers[] = get_string('report:heading:type', 'block_couponext');
            } else {
                $headers[] = get_string('report:heading:' . $name, 'block_couponext');
            }
        }
