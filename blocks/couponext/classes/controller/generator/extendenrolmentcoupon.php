...
/**
 * block_couponext\controller\generator\extendenrolmentcoupon
 *
 * @package     block_couponext
 *
 * @copyright   Sebsoft.nl
...
class extendenrolmentcoupon {
    ...
    protected function process_page_1() {
        ...
        if (!empty($courseid) && $courseid > 1) {
            ...
            if (!$DB->record_exists('course', array('id' => $courseid))) {
                redirect(new moodle_url($CFG->wwwroot . '/my'),
                        get_string('generator:extendenrolment:invalidcourse', 'block_couponext'));
            }
        ...
        // Create form.
        $mform = new \block_couponext\forms\coupon\extendenrolment\page1($url,
    ...
    protected function process_page_2() {
        ...
        $generatoroptions = generatoroptions::from_session();
        // Create form.
        $mform = new \block_couponext\forms\coupon\extendenrolment\page2($url, ['generatoroptions' => $generatoroptions]);
    ...
    protected function process_page_3() {
        ...
        $generatoroptions = generatoroptions::from_session();
        // Create form.
        $mform = new \block_couponext\forms\coupon\extendenrolment\page3($url, ['generatoroptions' => $generatoroptions]);
        ...
        if ($mform->is_cancelled()) {
            ...
            if (empty($generatoroptions->emailto)) {
                redirect(new moodle_url($CFG->wwwroot . '/my'), get_string('coupons_ready_to_send', 'block_couponext'));
            } else {
