...
/**
 * block_couponext\manager\cleanup
 *
 * @package     block_couponext
 *
 * @copyright   Sebsoft.nl
...
class cleanup {
    ...
    public function execute_request() {
        ...
        $url = new moodle_url($CFG->wwwroot . '/blocks/couponext/view/cleanup.php',
    ...
    protected function process_cleanup($owner) {
        ...
        // Create form.
        $mform = new \block_couponext\forms\coupon\cleanup($this->page->url, array('ownerid' => $owner));

        if ($mform->is_cancelled()) {
            redirect($redirect);
        } else if ($data = $mform->get_data()) {
            // Redirect to confirmation.
            $redirect = new moodle_url($CFG->wwwroot . '/blocks/couponext/view/cleanup.php',
    ...
    protected function process_confirm($owner) {
        ...
        $confirmdata = new \block_couponext\output\component\cleanupconfirm($options);
        $rendered = $this->renderer->render_from_template('block_couponext/cleanupconfirm',
                $confirmdata->export_for_template($this->renderer));

        $formoptions = [
            get_string('cleanup:confirm:header', 'block_couponext'),
            $rendered,
            get_string('cleanup:confirm:confirmmessage', 'block_couponext')
        ];
        $url = new moodle_url($CFG->wwwroot . '/blocks/couponext/view/cleanup.php',
                    array('id' => $this->page->url->param('id'), 'data' => base64_encode(json_encode($options)),
                        'action' => 'confirm'));
        $mform = new \block_couponext\forms\confirmation($url, $formoptions);
        if ($mform->is_cancelled()) {
            redirect($redirect);
        } else if ($formdata = $mform->get_data()) {
            if ((bool) $formdata->confirm) {
                $count = \block_couponext\helper::cleanup_coupons($options);
                \core\notification::success(get_string('coupons:cleaned', 'block_couponext', $count));
            }
            redirect($redirect);
        }
