...
/**
 * Request admin manager implementation for use with block_couponext
 *
...
/**
 * block_couponext\manager\requestadmin
 *
 * @package     block_couponext
 *
 * @copyright   Sebsoft.nl
...
class requestadmin {
    ...
    protected function process_user_overview() {
        $table = new \block_couponext\tables\requestusers();
        ...
        echo \html_writer::link($addurl, get_string('str:request:adduser', 'block_couponext'));
        echo '<br/>';
    ...
    protected function process_add_user() {
        ...
        $mform = new \block_couponext\forms\request\adduser($url);
    ...
    protected function process_delete_user() {
        ...
        $options = [
            get_string('delete:requestuser:header', 'block_couponext', $user),
            get_string('delete:requestuser:description', 'block_couponext', $user),
            get_string('delete:requestuser:confirmmessage', 'block_couponext', $user)
        ];
        $mform = new \block_couponext\forms\confirmation($url, $options);
        if ($mform->is_cancelled()) {
            redirect($redirect);
        } else if ($data = $mform->get_data()) {
    ...
    protected function process_request_overview() {
        $table = new \block_couponext\tables\requests();
    ...
    protected function process_deny_request() {
        ...
        $options = [
            get_string('request:deny:heading', 'block_couponext', $user),
            $this->renderer->requestdetails($instance)
        ];
        $mform = new \block_couponext\forms\request\deny($url, $options);
        if ($mform->is_cancelled()) {
            redirect($redirect);
        } else if ($data = $mform->get_data()) {
            ...
            \block_couponext\helper::do_email_to_user($user, $from, $subject, $messagetext, $messagehtml);
            redirect($redirect);
        }
        echo $this->output->header();
        echo $this->renderer->get_tabs($this->page->context, 'cprequests', $this->page->url->params());
    ...
    protected function process_accept_request() {
        ...
        $options = [
            get_string('request:accept:heading', 'block_couponext', $user),
            $this->renderer->requestdetails($instance)
        ];
        $mform = new \block_couponext\forms\request\accept($url, $options);
        if ($mform->is_cancelled()) {
            redirect($redirect);
        } else if ($data = $mform->get_data()) {
            ...
            try {
                ...
                \block_couponext\helper::mail_requested_coupons($user, $coupons,
