...
/**
 * block_couponext\manager\myrequests
 *
 * @package     block_couponext
 *
 * @copyright   Sebsoft.nl
...
class myrequests {
    ...
    protected function process_request_overview() {
        $table = new \block_couponext\tables\myrequests();
        ...
        echo \html_writer::link($newurl, get_string('str:request:add', 'block_couponext'));
        echo '<br/>';
    ...
    protected function process_batchlist_overview() {
        global $USER;
        // Table instance.
        $table = new \block_couponext\tables\downloadbatchlist($this->page->context, $USER->id);
    ...
    protected function process_delete_request() {
        ...
        $options = [
            get_string('delete:request:header', 'block_couponext', $user),
            $this->renderer->requestdetails($instance),
            get_string('delete:request:confirmmessage', 'block_couponext', $user)
        ];
    ...
    protected function assert_user($userid) {
        ...
        if (has_capability('block/couponext:administration', $this->page->context)) {
            // We will also allow anyone with administration rights.
            return;
        }
        if ($USER->id != $userid) {
            throw new \block_couponext\exception('error:myrequests:user');
        }
    }
