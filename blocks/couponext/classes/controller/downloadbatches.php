...
/**
 * block_couponext\controller\downloadbatches
 *
 * @package     block_couponext
 *
 * @copyright   Sebsoft.nl
...
class downloadbatches {
    ...
    public function execute_request() {
        ...
        $url = new moodle_url($CFG->wwwroot . '/blocks/couponext/view/batchlist.php',
                array('id' => $this->page->url->param('id'), 'tab' => 'cpbatchlist'));
        $this->page->set_url($url);

        $this->page->set_title(get_string($title, 'block_couponext'));
        $this->page->set_heading(get_string($title, 'block_couponext'));
    ...
    protected function process_viewlist() {
        ...
        // Table instance.
        $table = new \block_couponext\tables\downloadbatchlist($this->page->context, $ownerid);
