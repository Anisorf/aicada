...
namespace block_couponext\forms\coupon\extendenrolment;
...
/**
 * block_couponext\forms\coupon\extendenrolment\page2
 *
 * @package     block_couponext
 *
 * @copyright   Sebsoft.nl
...
class page2 extends \moodleform {
    ...
    protected function definition() {
        ...
        if (count($users) === 0) {
            // We cannot process.
            $err = get_string('extendenrol:abort-no-users', 'block_couponext');
            ...
            // User selection.
            $mform->addElement('static', '_extendusers', '', get_string('label:extendusers:desc', 'block_couponext'));
            $attributes = array('size' => min(10, max(4, count($users))));
            $select = $mform->addElement('select', 'extendusers',
                    get_string('label:users', 'block_couponext'), $users, $attributes);
            ...
            $mform->addElement('header', 'lastSettings',
                    get_string('heading:general_settings', 'block_couponext'));

            // Configurable redirect url.
            $mform->addElement('text', 'redirect_url',
                    get_string('label:redirect_url', 'block_couponext'), array('size' => 40));
            ...
            $mform->addHelpButton('redirect_url', 'label:redirect_url', 'block_couponext');

            // Configurable enrolment extension time.
            $mform->addElement('static', '_enrolperiod', '',
                    get_string('label:extendperiod:desc', 'block_couponext'));
            $mform->addElement('duration', 'enrolperiod',
                    get_string('label:extendperiod', 'block_couponext'), array('size' => 40, 'optional' => true));
            ...
            $mform->addElement('text', 'alternative_email',
                    get_string('label:alternative_email', 'block_couponext'), array('size' => 40));
            ...
            $mform->addElement('advcheckbox', 'generate_pdf', get_string('label:generate_pdfs', 'block_couponext'));
            ...
            $mailcontentdefault = get_string('coupon_mail_extend_content', 'block_couponext');
            $mform->addElement('editor', 'email_body_manual',
                    get_string('label:email_body', 'block_couponext'), array('noclean' => 1));
            ...
            $mform->addElement('date_selector', 'date_send_coupons_manual',
                    get_string('label:date_send_coupons', 'block_couponext'), $options);
