...
/**
 * block_couponext\forms\coupon\extendenrolment\page3
 *
 * @package     block_couponext
 *
 * @copyright   Sebsoft.nl
...
class page3 extends \moodleform {
    ...
    protected function definition() {
        ...
        $a = new \stdClass();
        $a->coupontype = get_string('coupon:type:enrolext', 'block_couponext');
        ...
        if (!empty($generatoroptions->emailto)) {
            ...
            $a->recipient = get_string('recipient:selected:users', 'block_couponext');
            // Display recipients.
            $recipients = [];
            ...
            $a->recipient = get_string('recipient:none', 'block_couponext');
        }
        $logos = \block_couponext\logostorage::get_file_menu();
        $a->logo = $logos[$generatoroptions->logoid];
        if (empty($generatoroptions->senddate)) {
            $a->senddate = get_string('coupon:senddate:instant', 'block_couponext');
        } else {
            $a->senddate = userdate($generatoroptions->senddate);
        }
        $a->emailbody = $generatoroptions->emailbody;

        $mform->addElement('static', '_confirmdesc', '', get_string('coupon:extenrol:summary', 'block_couponext', $a));
