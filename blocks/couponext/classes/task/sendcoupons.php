...
/**
 * block_couponext\task\sendcoupons
 *
...
class sendcoupons extends \core\task\scheduled_task {
    ...
    public function get_name() {
        return get_string('task:sendcoupons', 'block_couponext');
    }
    ...
    public function execute() {
        ...
        switch ($DB->get_dbfamily()) {
            case 'oracle':
                ...
                $sql = "SELECT batchid FROM(
                        SELECT batchid FROM {block_couponext}
                        WHERE senddate < ? AND issend = 0 AND for_user_email IS NOT NULL
                        ORDER BY timecreated ASC
                    ) where ROWNUM = 1";
                break;
            default:
                $sql = "SELECT batchid FROM {block_couponext}
                    WHERE senddate < ? AND issend = 0 AND for_user_email IS NOT NULL
                    ORDER BY timecreated ASC
                    LIMIT 1";
                break;
        ...
        $ownerid = $DB->get_field('block_couponext', 'ownerid', ['batchid' => $batchid], IGNORE_MULTIPLE);
    ...
    protected function send_batch($coupons, $batchid, $timeexecuted) {
        ...
        $batchcomplete = ($DB->count_records('block_couponext', $conditions) === 0);
        if ($batchcomplete) {
            // Mail confirmation.
            mtrace("Send batch completed notification");
            \block_couponext\couponnotification::send_task_notification($ownerid, $batchid, $timeexecuted);
        }
    }
