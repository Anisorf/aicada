...
namespace block_couponext\filters;
...
/**
 * block_couponext\filters\couponcoursegroupid
 *
 * @package     block_couponext
 *
 * @copyright   Sebsoft.nl
 ...
 * @license     http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class couponcoursegroupid extends \user_filter_type {
    /** @var string */
    protected $fieldid;
    ...
    public function __construct($advanced, $fieldid = 'id') {
        ...
        parent::__construct('couponcoursegroupid', get_string('group'), $advanced);
    }
    ...
    public function get_sql_filter($data) {
        global $DB;
        static $counter = 0;
        $name = 'ex_couponcoursegroupid'.$counter++;
        ...
        switch($operator) {
            case 0: // Contains.
                $res = $DB->sql_like('g.' . $field, ":$name", false, false);
                $params[$name] = "%$value%";
                break;
            ...
            case 5: // Empty.
                $not = 'NOT';
                $res = '(g.' . $field . ' IS NOT NULL AND g.' . $field . ' <> :'.$name.')';
                $params[$name] = '';
                break;
            default:
                return '';
        ...
        $sql = "{$this->fieldid} $not IN (SELECT couponid
                         FROM {block_couponext_groups} cg
