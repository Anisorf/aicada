...
class pdf extends \pdf {
     ...
     * @return \block_couponext\coupon\pdf
     */
    ...
    public function __construct($titlestring) {
        ...
        $this->SetTitle(get_string('pdf-meta:title', 'block_couponext'));
        $this->SetSubject(get_string('pdf-meta:subject', 'block_couponext'));
        $this->SetKeywords(get_string('pdf-meta:keywords', 'block_couponext'));
    ...
    protected function set_coupon_logo($coupon) {
        ...
        switch ($coupon->logoid) {
            ...
            case 0:
                // Default.
                $this->logos[$coupon->logoid] = $CFG->dirroot . '/blocks/couponext/pix/couponlogo.png';
            ...
            default:
                // File ID.
                $tempfile = \block_couponext\logostorage::get_tempfile_for($coupon->logoid);
    ...
    protected function write_coupon_pages() {
        ...
        foreach ($this->coupons as $coupon) {
            ...
            // QR.
            if ((bool)$coupon->renderqrcode) {
                $url = new \moodle_url($CFG->wwwroot . '/blocks/couponext/view/qrin.php', array(
    ...
    protected function compile_main($coupon, $courses) {
        ...
        if ((int)$coupon->enrolperiod === 0) {
            $accesstime = get_string('unlimited_access', 'block_couponext');
        } else if ($coupon->typ == generatoroptions::ENROLEXTENSION ) {
            $accesstime = get_string('extendaccess', 'block_couponext', format_time($coupon->enrolperiod));
        } else {
