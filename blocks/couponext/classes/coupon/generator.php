 ...
 * Encoding     UTF-8
 *
 * @package     blockblock_couponext_coupon
 *
...
use block_couponext\coupon\codegenerator;
use block_couponext\coupon\icoupongenerator;
use block_couponext\exception;

/**
 * block_couponext\coupon\generator
 *
    ...
    protected function validate_courses($courseids, $groupids = null) {
        ...
        foreach ($courseids as $courseid) {
            if (!isset($this->courses[$courseid])) {
                $errors[] = get_string('error:course-not-found', 'block_couponext') . ' (id = ' . $courseid . ')';
            }
        }
        // Groups.
        if (!empty($groupids)) {
            ...
            foreach ($groupids as $groupid) {
                if (!isset($this->groups[$groupid])) {
                    $errors[] = get_string('error:group-not-found', 'block_couponext') . ' (id = ' . $groupid . ')';
                }
            }
        }
        // Do we have errors?
    ...
    protected function generate_email($template, $coupon) {
        ...
        if ($coupon->typ == generatoroptions::ENROLEXTENSION) {
            if (empty($coupon->enrolperiod)) {
                $extensionperiod = get_string('enrolperiod:indefinite', 'block_couponext');
            } else {
                $extensionperiod = get_string('enrolperiod:extension', 'block_couponext', format_time($coupon->enrolperiod));
            }
        }
