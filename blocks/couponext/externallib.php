...
/**
 * Webservices implementation for block_couponext
 *
...
class block_coupon_external extends external_api {
    ...
    public static function get_courses() {
        $rs = \block_couponext\helper::get_visible_courses('id,shortname,fullname,idnumber');
    ...
    public static function get_cohorts() {
        $rs = \block_couponext\helper::get_cohorts('id,name,idnumber');
        return array_values($rs);
    }
    ...
    public static function generate_coupons_for_course($email, $amount, $courses, $groups = null, $generatesinglepdfs = false) {
        ...
        $coupons = $DB->get_records_list('block_couponext', 'submission_code', $generatedcodes);
        $status = block_couponext\helper::mail_coupons($coupons, $email, $generatesinglepdfs,
    ...
    static public function get_coupon_reports($type = 'all', $ownerid = null, $fromdate = null, $todate = null) {
        ...
        $courseids = $DB->get_fieldset_select('block_couponext_courses', 'courseid', "couponid $cinsql", $cparams);
        $cohortids = $DB->get_fieldset_select('block_couponext_cohorts', 'cohortid', "couponid $cinsql", $cparams);
        ...
        foreach ($coupons as $coupon) {
            ...
            switch ($coupon->typ) {
                case \block_couponext\coupon\generatoroptions::COURSE:
                case \block_couponext\coupon\generatoroptions::COURSESPECIFIC:
                case \block_couponext\coupon\generatoroptions::ENROLEXTENSION:
                    // Load courses.
                    $cids = $DB->get_fieldset_select('block_couponext_courses', 'courseid', 'couponid = ?', array($coupon->id));
                    ...
                    foreach ($cids as $courseid) {
                        ...
                        if (!empty($report->userid)) {
                            ...
                            $completioninfo = block_couponext\helper::load_course_completioninfo($user, $courses[$courseid]);
                ...
                case \block_couponext\coupon\generatoroptions::COHORT:
                    // Load cohorts.
                    $cids = $DB->get_fieldset_select('block_couponext_cohorts', 'cohortid', 'couponid = ?', array($coupon->id));
    ...
    public static function find_users($query) {
        ...
        $where[] = 'u.id NOT IN (SELECT userid FROM {block_couponext_rusers})';
    ...
    static private function _request_coupon_codes_for_course($amount, $courses, $groups = null) {
        ...
        $generatoroptions = new \block_couponext\coupon\generatoroptions();
        $generatoroptions->type = \block_couponext\coupon\generatoroptions::COURSE;
        ...
        if ($rs !== true) {
            ...
            throw new block_couponext\exception('err:generating:coupons', implode("\n", $errors));
        }
    ...
    static private function _request_coupon_codes_for_cohorts($amount, $cohorts) {
        ...
        // Get max length for the coupon code.
        if (!$couponcodelength = get_config('couponext', 'coupon_code_length')) {
            $couponcodelength = 16;
        }
        $generatoroptions = new \block_couponext\coupon\generatoroptions();
        $generatoroptions->type = \block_couponext\coupon\generatoroptions::COHORT;
        $generatoroptions->amount = $amount;
        $generatoroptions->codesize = $couponcodelength;
     ...
     */
    static private function _request_coupon_codes($amount, $groups = null) {
        global $USER;
        ...
        // Initialize generator options.
        $generatoroptions = new \block_couponext\coupon\generatoroptions();
        $generatoroptions->type = \block_couponext\coupon\generatoroptions::COURSESPECIFIC;
        $generatoroptions->amount = $amount;
        $generatoroptions->codesize = $couponcodelength;
        $generatoroptions->cohorts = [];
        $generatoroptions->courses = null;
        $generatoroptions->csvrecipients = [];
        $generatoroptions->emailbody = '';
        $generatoroptions->emailto = '';
        $generatoroptions->enrolperiod = 0;// TODO: add enrolperiod?
