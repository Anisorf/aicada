...
/**
 * Upgrade script for block_couponext
 *
 * File         upgrade.php
 * Encoding     UTF-8
 *
 ...
 * @return boolean
 */
function xmldb_block_couponext_upgrade($oldversion) {
    global $DB, $CFG;
    $dbman = $DB->get_manager();

    if ($oldversion < 2016011000) {
        // Add activity completion table.
        $table = new xmldb_table('block_couponext_errors');
        ...
        $table = new xmldb_table('block_couponext_cohorts');
        ...
        $table = new xmldb_table('block_couponext_groups');
        $index = new xmldb_index('couponid', XMLDB_INDEX_NOTUNIQUE, array('couponid'));
        if ($dbman->index_exists($table, $index)) {
            $dbman->add_index($table, $index);
        }
        ...
        $table = new xmldb_table('block_couponext_courses');
        $index = new xmldb_index('couponid', XMLDB_INDEX_NOTUNIQUE, array('couponid'));
        if ($dbman->index_exists($table, $index)) {
            $dbman->add_index($table, $index);
        }
    ...
    if ($oldversion < 2017050100) {
        // Add activity completion table.
        $table = new xmldb_table('block_couponext');
        ...
        // Set cohort types.
        $cids = $DB->get_fieldset_sql('SELECT DISTINCT couponid FROM {block_couponext_cohorts}');
        list($insql, $params) = $DB->get_in_or_equal($cids, SQL_PARAMS_QM, 'unused', true, 0);
        array_unshift($params, 'cohort');
        $DB->execute('UPDATE {block_couponext} SET typ = ? WHERE id '.$insql, $params);
        // Set course types.
        list($notinsql, $params) = $DB->get_in_or_equal($cids, SQL_PARAMS_QM, 'unused', false, 0);
        array_unshift($params, 'course');
        $DB->execute('UPDATE {block_couponext} SET typ = ? WHERE id '.$notinsql, $params);
        ...
        if (file_exists($logofile)) {
            ...
            \block_couponext\logostorage::store_from_content('couponlogo.png', $content);
            // Delete original.
            unlink($logofile);
            // ANd remove dir.
    ...
    if ($oldversion < 2017050102) {
        ...
        $sql = 'UPDATE {block_couponext} SET claimed = 1 WHERE (userid IS NOT NULL OR userid = 0)';
        $DB->execute($sql);
    ...
    if ($oldversion < 2017050103) {
        ...
        $sql = 'UPDATE {block_couponext} SET enrolperiod = enrolperiod * 86400 WHERE enrolperiod <> 0';
        $DB->execute($sql);
    ...
    if ($oldversion < 2018050301) {
        ...
        $table = new xmldb_table('block_couponext_rusers');
        $table->add_field('id', XMLDB_TYPE_INTEGER, '11', XMLDB_UNSIGNED, XMLDB_NOTNULL, XMLDB_SEQUENCE, null);
        ...
        $table = new xmldb_table('block_couponext_requests');
        $table->add_field('id', XMLDB_TYPE_INTEGER, '11', XMLDB_UNSIGNED, XMLDB_NOTNULL, XMLDB_SEQUENCE, null);
        $table->add_field('userid', XMLDB_TYPE_INTEGER, '11', null, XMLDB_NOTNULL, null, null, 'id');
        $table->add_field('configuration', XMLDB_TYPE_TEXT, 'medium', null, XMLDB_NOTNULL, null, null, 'userid');
        $table->add_field('timecreated', XMLDB_TYPE_INTEGER, '18', null, XMLDB_NOTNULL, null, null, 'configuration');
    ...
    if ($oldversion < 2018050303) {
        ...
        $DB->execute("UPDATE {block_couponext} SET timeclaimed = timemodified WHERE claimed = 1");
