...
class block_couponext_renderer extends plugin_renderer_base {
    ...
    public function requestdetails($request) {
        $widget = new \block_couponext\output\component\requestdetails($request);
        return $this->render_requestdetails($widget);
    }

    /**
     * Render request details
     * @param \block_couponext\output\component\requestdetails $widget
     */
    public function render_requestdetails(\block_couponext\output\component\requestdetails $widget) {
        $context = $widget->export_for_template($this);
        return $this->render_from_template('block_couponext/requestdetails', $context);
    }
    ...
    public function page_report($id, $ownerid = null) {
        // Table instance.
        $table = new \block_couponext\tables\report($ownerid);
        $table->baseurl = $this->page->url;

        $filtering = new \block_couponext\tablefilters\report($this->page->url);
    ...
    public function get_tabs($context, $selected, $params = array()) {
        ...
        $tabs[] = $this->create_pictab('wzcoupons', 'e/print', '',
                new \moodle_url($CFG->wwwroot . '/blocks/couponext/view/generator/index.php', $params),
                get_string('tab:wzcoupons', 'block_couponext'));
        $tabs[] = $this->create_pictab('wzcouponimage', 'e/insert_edit_image', '',
                new \moodle_url($CFG->wwwroot . '/blocks/couponext/view/managelogos.php', $params),
                get_string('tab:wzcouponimage', 'block_couponext'));

        $requesttab = $this->create_pictab('cprequestadmin', 'i/checkpermissions', '',
                new \moodle_url($CFG->wwwroot . '/blocks/couponext/view/requests/admin.php', $params),
                get_string('tab:requests', 'block_couponext'));
        $requesttab->subtree[] = $this->create_pictab('cprequestusers', 'i/users', '',
                new \moodle_url($CFG->wwwroot . '/blocks/couponext/view/requests/admin.php', $params + ['action' => 'users']),
                get_string('tab:requestusers', 'block_couponext'));
        $requesttab->subtree[] = $this->create_pictab('cprequests', 'e/help', '',
                new \moodle_url($CFG->wwwroot . '/blocks/couponext/view/requests/admin.php', $params + ['action' => 'requests']),
                get_string('tab:requests', 'block_couponext'));
        $tabs[] = $requesttab;

        $tabs[] = $this->create_pictab('cpreport', 'i/report', '',
                new \moodle_url($CFG->wwwroot . '/blocks/couponext/view/reports.php', $params),
                get_string('tab:report', 'block_couponext'));
        $tabs[] = $this->create_pictab('cpunused', 'i/completion-manual-n', '',
                new \moodle_url($CFG->wwwroot . '/blocks/couponext/view/couponview.php',
                array_merge($params, array('tab' => 'unused'))),
                get_string('tab:unused', 'block_couponext'));
        $tabs[] = $this->create_pictab('cpused', 'i/completion-manual-enabled', '',
                new \moodle_url($CFG->wwwroot . '/blocks/couponext/view/couponview.php',
                array_merge($params, array('tab' => 'used'))),
                get_string('tab:used', 'block_couponext'));
        $tabs[] = $this->create_pictab('cperrorreport', 'i/warning', '',
                new \moodle_url($CFG->wwwroot . '/blocks/couponext/view/errorreport.php',
                array_merge($params, array('tab' => 'cperrorreport'))),
                get_string('tab:errors', 'block_couponext'));
        $tabs[] = $this->create_pictab('cpcleaner', 'e/cleanup_messy_code', '',
                new \moodle_url($CFG->wwwroot . '/blocks/couponext/view/cleanup.php',
                array_merge($params, array('tab' => 'cpcleaner'))),
                get_string('tab:cleaner', 'block_couponext'));
        $tabs[] = $this->create_pictab('cpbatchlist', 'i/down', '',
                new \moodle_url($CFG->wwwroot . '/blocks/couponext/view/downloadbatchlist.php',
                array_merge($params, array('tab' => 'cpbatchlist'))),
                get_string('tab:downloadbatchlist', 'block_couponext'));
        $tabs[] = $this->create_pictab('cpmaillog', 'i/warning', '',
                new \moodle_url($CFG->wwwroot . '/blocks/couponext/view/maillog.php',
                array_merge($params, array('tab' => 'maillog'))),
                get_string('tab:maillog', 'block_couponext'));
    ...
    public function get_my_requests_tabs($context, $selected, $params = array()) {
        ...
        $requesttab = $this->create_pictab('cpmyrequests', 'i/checkpermissions', '',
                new \moodle_url($CFG->wwwroot . '/blocks/couponext/view/requests/userrequest.php', $params),
                get_string('tab:requests', 'block_couponext'));
        $requesttab->subtree[] = $this->create_pictab('myrequests', null, '',
                new \moodle_url($CFG->wwwroot . '/blocks/couponext/view/requests/userrequest.php',
                        $params + ['action' => 'list']),
                get_string('tab:listrequests', 'block_couponext'));
        switch ($selected) {
            case 'newrequest':
                $requesttab->subtree[] = $this->create_pictab('newrequest', null, '',
                        new \moodle_url($CFG->wwwroot . '/blocks/couponext/view/requests/userrequest.php',
                                $params + ['action' => 'newrequest']),
                        get_string('str:request:add', 'block_couponext'));
                break;
            case 'delete':
                $requesttab->subtree[] = $this->create_pictab('delete', null, '',
                        new \moodle_url($CFG->wwwroot . '/blocks/couponext/view/requests/userrequest.php',
                                $params + ['action' => 'delete']),
                        get_string('delete:request:header', 'block_couponext'));
                break;
            case 'details':
                $requesttab->subtree[] = $this->create_pictab('details', null, '',
                        new \moodle_url($CFG->wwwroot . '/blocks/couponext/view/requests/userrequest.php',
                                $params + ['action' => 'details']),
                        get_string('str:request:details', 'block_couponext'));
                break;
        }
        $tabs[] = $requesttab;
