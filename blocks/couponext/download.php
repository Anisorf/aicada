...
$PAGE->navbar->add(get_string($title, 'block_couponext'));

$params = ['id' => $zipid];
$url = new moodle_url($CFG->wwwroot . '/blocks/couponext/download.php', $params);
$PAGE->set_url($url);

$PAGE->set_title(get_string($title, 'block_couponext'));
$PAGE->set_heading(get_string($heading, 'block_couponext'));
...
function download($batchid, $timeid, $dodl = false) {
    ...
    if (file_exists("{$CFG->dataroot}/{$basename}.zip")) {
        ...
        throw new block_couponext\exception('err:download-not-exists', $a);
    }

    if (!$dodl) {
        $url = new \moodle_url($CFG->wwwroot . '/blocks/couponext/download.php', ['bid' => $batchid, 't' => $timeid, 'dl' => 1]);
        $button = $OUTPUT->single_button($url, get_string('downloadcoupons:buttontext', 'block_couponext'), 'get');
        echo get_string('downloadcoupons:text', 'block_couponext', $button);
    } else {
