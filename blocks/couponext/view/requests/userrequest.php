...
$url = new moodle_url($CFG->wwwroot . '/blocks/couponext/view/requests/userrequest.php', array('id' => $id));
$PAGE->set_url($url);
...
if (!$DB->record_exists('block_couponext_rusers', ['userid' => $USER->id])) {
    throw new \block_couponext\exception('err:not-a-requestuser');
}
...
$controller = new \block_couponext\controller\myrequests($PAGE, $OUTPUT, $renderer);
