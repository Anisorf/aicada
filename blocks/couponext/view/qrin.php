...
try {
    $data = $DB->get_record('block_couponext', array('submission_code' => $code), '*', IGNORE_MISSING);
    if (empty($data->id)) {
        throw new block_couponext\exception('error:invalid_coupon_code');
    } else if ($hash !== sha1($data->id . $data->ownerid . $data->submission_code)) {
        throw new block_couponext\exception('error:invalid_coupon_code');
    } else if (!empty($data->userid)) {
        throw new block_couponext\exception('error:coupon_already_used');
    } else {
        if (!isloggedin()) {
            // Redirect to signup with coupon code.
            $params = array('submissioncode' => $data->submission_code);
            $couponsignup = new \moodle_url($CFG->wwwroot . '/blocks/couponext/view/signup.php', $params);
            redirect($couponsignup);
            exit; // Never reached.
        }
        require_login(null, false);
        // Get type processor.
        $typeproc = block_couponext\coupon\typebase::get_type_instance($data->submission_code);
        // Perform assertions.
